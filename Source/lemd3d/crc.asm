; crc32.asm
; MASM 6.x / Intel
; Shaun Wilson, May 2001
;
; This module provides a crc32 implementation necessary to provide crc32 
; support in any number of applications
;
; Thanks to LL for providing me the crc table values and thanks to ASEC
; (see http://www.arturocampos.com/ac_crc32.html) for the CRC-32 
; Pseudo-code and explanations. Special thanks to ludde for providing
; many optimization ideas that brought the speed from ~1-2 seconds for
; 5megs of data down to ~300-600ms. (ludde of EFnet's #asm)
;
; a C Declaration for crc32
; extern unsigned long crc32(char* data, unsigned long length);
;
...
.code
crc32 PROC C data:DWORD, ln:DWORD 
	mov eax, 0ffffffffh		; crc value
	mov ebx, data			; data indexer
	xor ecx, ecx			; temp
	mov edx, ln				; counter
	lea esi, @CRC32TABLE	; table
@0:
	mov cl, [ebx] 
	xor cl, al				 
	inc ebx
...
@CRC32TABLE:				; crc32 lookup table
	...
	dd 0b40bbe37h, 0c30c8ea1h, 05a05df1bh, 02d02ef8dh, 000706c65h, 058757300h
crc32 ENDP
